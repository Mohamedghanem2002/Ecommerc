...
const Header = () => {
  ...
  const toggleMenu = () => setMenuOpned((prev) => !prev);
  return (
        ...
        <div className="flex-1 flex items-center justify-end gap-x-2 xs:gap-x-8">
          ...
          <Link to={"/cart"} className="flex relative">
            <div className="ring-1 ring-slate-900 rounded-full px-3 bold-18">
              Cart
              <span
                className="bg-secondary text-white text-[12px]
                font-semibold absolute -top-3.5 -right-2 flexCenter w-4 h-4 rounded-full shadow-md"
              >
                {getCartCount()}
              </span>
            </div>
